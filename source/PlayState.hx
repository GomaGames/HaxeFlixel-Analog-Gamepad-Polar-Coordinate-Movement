...
class PlayState extends FlxState
{
  ...
  private inline function handleAnimation():Void
  {
     ...
     */
    var angle = FlxAngle.getAngle(FlxPoint.weak(0,0), FlxPoint.weak(shape_velocity.x, shape_velocity.y)) - 90;

    /*
      Get the speed based on the analog pressure or "distance" of the joystick
      Math.sqrt((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));
     */
    var speed = FlxMath.getDistance(FlxPoint.weak(0,0), FlxPoint.weak(shape_velocity.x, shape_velocity.y));

    /*
      Move the shape
      http://api.haxeflixel.com/flixel/util/FlxVelocity.html#velocityFromAngle
     */
    var velocity = FlxVelocity.velocityFromAngle(angle, speed * Reg.MOVE_SPEED);
  ...
  private inline function updateGamepadMovement():Void
  {
    ...
    if( game_pad.getAxis(LogitechButtonID.LEFT_ANALOGUE_Y) != 0 ){
      shape_velocity.y = game_pad.getYAxis(LogitechButtonID.LEFT_ANALOGUE_Y);
    }else{
    ...
    if( game_pad.getAxis(LogitechButtonID.LEFT_ANALOGUE_X) != 0 ){
      shape_velocity.x = game_pad.getXAxis(LogitechButtonID.LEFT_ANALOGUE_X);
    }else{
